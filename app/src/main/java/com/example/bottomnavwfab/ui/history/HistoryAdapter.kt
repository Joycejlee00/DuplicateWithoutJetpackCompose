    ...
    override fun getFilter(): Filter {
        return object: Filter(){
            ...
            override fun publishResults(p0: CharSequence?, p1: FilterResults?) {
                historyList = p1?.values as ArrayList<Recipe> /* = java.util.ArrayList<com.example.bottomnav.data.RecipeData> */
                notifyDataSetChanged()
            }
        }
    }
        ...
        historyList?.let { list ->
            with(holder) {
                with(list[position]) {
                    ...
                    binding.tvHistoryTitle.text = historyList?.get(position)?.title
                    binding.tvRecipeIngredientsHistory.text = historyList?.get(position)?.ingredients
